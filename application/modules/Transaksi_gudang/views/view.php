          ...
          <div class="multi-filter">
            ...
            <div class="filter-item">
              <select class="form-control" name="" id="filter_bahan" data-column="1">
                <option value="" data-filter="">Filter Bahan</option>
                <?php foreach($list_bahan as $bahan): ?> ?>
                <option value="<?= $bahan->id ?>" data-filter="<?= $bahan->id ?>"><?= $bahan->nama ?></option>
                <?php endforeach; ?>
           ...
           <!--  <div class="filter-item">
            ...
            <div class="filter-item">
              <select class="form-control" name="" id="filter_kategori" data-column="1">
                <option value="" data-filter="">Filter Kategori</option>
                <?php foreach($list_kategori as $kategori): ?> ?>
                  <option value="<?= $kategori->id ?>" data-filter="<?= $kategori->id ?>"><?= $kategori->nama ?></option>
                <?php endforeach; ?>
          ...
          <div class="col-xs-12">
            <div id="customdate">
              <div class="input-daterange input-group input-group-lg" id="datepicker">
                <input type="text" id="tanggalMulai" class="input-sm form-control" name="start" placeholder="Mulai Tanggal"/>
                <span class="input-group-addon">s/d</span>
                <input type="text" id="tanggalSelesai" class="input-sm form-control" name="end" placeholder="Sampai Tanggal"/>
...
$(document).ready(function() {
  $('#TableMainServer').DataTable({
    ...
    "ajax":{
          ...
          data: function(d) {
            d.id_bahan = $('#filter_bahan option:selected').attr('data-filter') || '';
            d.id_kategori = $('#filter_kategori option:selected').attr('data-filter') || '';
            // d.id_supplier = $('#filter_supplier option:selected').attr('data-filter') || '';
          },
  ...
  $('.filter-item select').on("change", function(event){
      $('#TableMainServer').DataTable().clear().draw();
  });
...
function cetakDaily(){
  window.open("<?php echo base_url()?>index/modul/Transaksi_gudang-master-cetak?tipe=harian", "_blank");
}
function cetakWeekly(){
  window.open("<?php echo base_url()?>index/modul/Transaksi_gudang-master-cetak?tipe=mingguan", "_blank");
}
function cetakMonthly(){
  window.open("<?php echo base_url()?>index/modul/Transaksi_gudang-master-cetak?tipe=bulanan", "_blank");
}
function cetakCustom(){
  var tglMulai = $('#tanggalMulai').val() || '';
  var tglSelesai = $('#tanggalSelesai').val() || '';
  window.open("<?php echo base_url()?>index/modul/Transaksi_gudang-master-cetak?tipe=custom&start="+tglMulai+"&end="+tglSelesai, "_blank");
}
