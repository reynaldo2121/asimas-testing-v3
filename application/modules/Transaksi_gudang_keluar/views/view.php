          ...
          <div class="row">
            ...
            <div class="col-sm-6" id="noBatch">
              <div class="form-group">
                ...
                <select name="no_batch" id="no_batch" class="form-control" onchange="selectExpiredDate(this.value)">
                </select>
              </div>
            </div>
            <div class="col-sm-6">
...
var list_batch = <?php echo json_encode($list_batch); ?>;
...
function selectExpiredDate(val='') {
  if(val) {
    let filter = list_batch.filter(function(index) {return index.no_batch == val});
    let data = filter.length > 0 ? filter[0] : false;
    if(data) {
      // console.log(data.expired_date);
      let date = new Date(data.expired_date);
      let d = date.getDate();
      let m = date.getMonth() + 1;
      let y = date.getFullYear();
      $('#expired_date').datepicker('setDate', d+'/'+m+'/'+y);
    }
    else {
      $('#expired_date').datepicker('setDate', '');
    }
  }
}
