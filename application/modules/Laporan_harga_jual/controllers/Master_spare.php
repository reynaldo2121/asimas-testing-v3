        ...
        foreach($query->result() as $bahan) {
            ...
            if(in_array($bahan->id, $availBahan)) {
                $sql = "SELECT gudang.id, gudang.id_bahan, bahan.nama,
                        ...
                        LEFT JOIN tt_gudang_masuk gm ON gm.id = gudang.id_gudang
                        LEFT JOIN m_bahan bahan ON gudang.id_bahan = bahan.id
                        LEFT JOIN tt_bahan tbahan ON tbahan.id_bahan = bahan.id
                        WHERE type = 1
                        AND gudang.id_bahan = {$bahan->id} ORDER BY gudang.date_add DESC";
                $query = $this->Laporanhargajualmodel->rawQuery($sql);
