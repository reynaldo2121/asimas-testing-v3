        ...
        <div class="form-group">
          ...
          <div class="col-sm-9">
            <select name="nama_produk" class="form-control" id="nama_produk" onchange="appendContent(this.options[this.selectedIndex].getAttribute('data-id'))">
              ...
              <option value="<?= $produk->nama_produk ?>" data-id="<?= $produk->id ?>"><?= $produk->nama_produk." (Rev. ".$produk->revisi.")" ?></option>
              <?php endforeach; ?>
            </select>
    ...
    <div class="panel panel-default">
      <div class="panel-body">
        ...
        <div class="form-group">
          ...
          <div class="col-sm-9">
            <input type="number" min="0" name="revisi" id="revisi" class="form-control">
...
function appendContent(id) {
  ...
  $.ajax({
      ...
      success: function (data) {
        ...
        $("#kode_produksi").val(perintah_produksi.kode_produksi);
        ...
        if(pp_bahan_baku != null) {
          ...
          for(i = 0; i < pp_bahan_baku.length; i++){
            ...
            var satuanBatch = getMasterById(list_satuan, data.satuan_batch);
            tempBahanBaku.push({
                'num': num,
            ...
            +"<td>Per Batch: "+data.per_batch+''+satuanBatch.nama+"</td>"
        ...
        if(pp_bahan_kemas != null) {
          ...
          for(i = 0; i < pp_bahan_kemas.length; i++){
            ...
            var satuanKemas = getMasterById(list_satuan, data.satuan);
            tempBahanKemas.push({
                'num': num,
                'id_bahan': data.id_bahan,
                'jumlah': data.jumlah,
            ...
            .append("<tr id='bahankemas"+num+"'>"
              +"<td>"+ data.nama_bahan +"</td><td>Jumlah: "+data.jumlah+''+satuanKemas.nama+"</td>"
              +"<td>"
                +"<div class='btn-group'>"
...
$("#formPerintahProduksi").on('submit', function(e){
  ...
  $.ajax({
      url: action,
      type: 'post',
      data: form+"&bahan_baku="+JSON.stringify(tempBahanBaku)+"&bahan_kemas="+JSON.stringify(tempBahanKemas)+"&revisi="+$("#revisi").val(),
      dataType: 'json',
      beforeSend: function() {
      ...
      success: function (data) {
        ...
        $("#revisi").val(parseInt($list.revisi) + parseInt(1));
        // Clear Field
        // numBahanBaku = 1;
        // numBahanKemas = 1;
        // tempBahanBaku = [];
