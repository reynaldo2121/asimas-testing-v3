...
class Master extends MX_Controller {
  ...
  function perintahbaru(){
    ...
    bahan.id, bahan.nama AS nama_bahan,  kategori.nama AS nama_kategori
    FROM m_bahan bahan, m_bahan_kategori kategori
    WHERE bahan.id_kategori_bahan = kategori.id 
    AND 
    (kategori.nama LIKE '%bahan baku%'
      or
      kategori.nama LIKE '%bahan cair%' 
      or
      kategori.nama LIKE '%bahan lain-lain%' 
      or
      kategori.nama LIKE '%bahan tambahan%' 
      or
      kategori.nama LIKE '%suhu terkontrol%' 
      )AND bahan.deleted = 1
...
function addData(){
  ...
  $dataInsert['tanggal_efektif'] = $params['tanggal_efektif'] ? date('Y-m-d', strtotime($params['tanggal_efektif'])) : date('Y-m-d');
...
function editData(){
  ...
  if($session_detail->id == 9) {
    ...
    $dataUpdate['tanggal_efektif'] = $params['tanggal_efektif'] ? date('Y-m-d', strtotime($params['tanggal_efektif'])) : $perintahProduksi->tanggal_efektif;
...
function getPerintahProduksi(){
  ...
  $perintahProduksi->tanggal_efektif = date('d.m.Y', strtotime($perintahProduksi->tanggal_efektif));
      // Bahan Baku & Penimbangan Aktual
  $bahanBaku = $this->Perintahproduksimodel->select(array('id_perintah_produksi' => $id), 'pp_bahan_baku')->result();
  $dataBahanBaku = null;
  foreach($bahanBaku as $row) {
